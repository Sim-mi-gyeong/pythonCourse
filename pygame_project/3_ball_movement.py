...
# 공 크기에 따른 최초 스피드 -> 공이 작을수록 아래쪽에서 튀어 오르게 됨.
ball_speed_y = [-18, -15, -12, -9]   # index 0, 1, 2, 3에 해당하는 값 - 공이 튀어오를 경우 y값을 빼주어야 하기때문에 음수
...
balls.append({
    ...
    "to_y": -6,    # y축 이동방향 -> # 처음 공의 위치가 살짝 올라갔다가 내려오는 효과를 줌
    "init_spd_y" : ball_speed_y[0]   # y 최초 속도 (공마다 ball_speed_y 의 값 중 어떤 것을 쓸지 정의)
})
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width:   # 반대쪽으로 튕겨지도록
            ball_val["to_x"] = ball_val["to_x"] * (-1)   # 반대로 튕겨주기(왼쪽으로 튕겨지다가 -> 충돌해서 -> 오른쪽으로 튕기기)

        # 세로 위치
        # 스테이지에 튕겨서 올라가는 처리
        if ball_pos_y >= screen_height - stage_height - ball_height:  # 공의 y좌표가 stage에 닿으면
            ball_val["to_y"] = ball_val["init_spd_y"]   # 최초로 튕기는 값 사용
        # stage에 닿는 경우가 아니라면 올라갈 땐 속도를 줄이고, 내려 올 땐 속도를 점점 빠르게 해줌. -> 포물선을 그리도록 
        else:   
            ball_val["to_y"] += 0.5   # -6 -> -5.5 -> -5 -> ... -> 0 -> 0.5 -> 1.0 -> ... 처럼 점점 빨리 내려오는 효과
